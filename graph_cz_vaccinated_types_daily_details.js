...
for (var i=0; i<used_vaccines.length; i++) {
    ...
    vaccinated_dataset.push( {
        label: vaccine + ' (1st)',
        data: current_values['cz_vaccinated_a_daily_' + vaccine],
        spanGaps: true,
        borderWidth: 1,
    ...
    vaccinated_dataset.push( {
        label: vaccine + ' (2nd)',
        data: current_values['cz_vaccinated_b_daily_' + vaccine],
        spanGaps: true,
        borderWidth: 2,
...
window.vaccinated_chart = new Chart(vaccinated_chart_cz_types_det_daily, {
    ...
    options: {
        ...
        title: {
            ...
            fontSize: fontsize,
            text: ["Confirmed vaccinations against COVID-19 in Czech Republic (breakdown by vaccine types, daily numbers)", "(click on the legend to show/hide data)"]
        },
        scales: {
            xAxes: [{
                gridLines: {
