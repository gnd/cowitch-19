...
// the growth rate
var vaccinated_chart_cz_det = document.getElementById("vaccinated_cz_det").getContext('2d');
...
for (var i=0; i<age_slots.length; i++) {
    ...
    vaccinated_dataset.push( {
        label: age_desc + ' (2nd)',
        data: current_values['cz_vaccinated_b_' + age],
        spanGaps: true,
        borderWidth: 2,
...
window.vaccinated_chart = new Chart(vaccinated_chart_cz_det, {
    ...
    options: {
        ...
        title: {
            ...
            fontSize: fontsize,
            text: ["Confirmed vaccinations against COVID-19 in Czech Republic (breakdown by age groups)", "(click on the legend to show/hide data)"]
        },
        scales: {
            xAxes: [{
                gridLines: {
